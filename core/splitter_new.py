...
# except Exception, e:
#     logger.debug('get user_list_special error:%s' % traceback.format_exc())
...
def submit(refresh_task, urls):
    ...
    try:
        ...
        if web_task:
            try:
                webluker_tools.post_data_to_webluker(web_task.get('task_new'), web_task.get('task_all'), str(web_task.get('r_id')))
            except Exception:
                logger.debug('splitter_new  webluker task have r_id:%s, error:%s' % (web_task.get('r_id'), traceback.format_exc()))
...
def process(db, refresh_task, check_overload=False):
    ...
    try:
        ...
        if check_overload:
            ...
            logger.debug('url_length:%s dir_length:%s' % (str(url_length), str(dir_length)))
            if (url_length > 0 and url_overload > 0):
                ...
                logger.debug('process error ! refresh_task :%s,url:%s' % (refresh_task['r_id'], url_overload))

            if (dir_length > 0 and dir_overload > 0):
                message['dirExceed'] = dir_length
                refresh_task['dirs'] = []
                setCounterCache(refresh_task, dir_length, 'DIR')
                logger.debug('process error ! refresh_task :%s,dir:%s ' % (refresh_task['r_id'], dir_overload))
    ...
    if message.get('invalids'):
        logger.debug('process error ! refresh_task :%s,invalid_urls:%s ' % (refresh_task['r_id'], len(invalid_urls)))
    return message
    ...
    try:
        ...
        if judge_contain_chinese(u.get('url')):
            for encode_t in chinese_encode:
                logger.debug("init_chinese_url chinese_encode encode_t:%s" % encode_t)
...
def noticeEmail(refresh_task):
    try:
        ...
        if callback.get('email'):
            # judge the uername is in redis(black list) or not, if not,  continue
