...
RETRY_DELAY_TIME = 3
RETRY_COUNT = 2
logger = log_utils.get_transfer_cert_postal_Logger()

def doloop_url(devs, tasks, Interface,connect_timeout=5, response_timeout=8):

    """
    调用AioClient，创建信道，与设备通过socket连接,端口51108
    :param devs:
    :param command:
    :return:
    """
...
def process_loop_ret(http_results, dev_map):
    ...
    for result in http_results:
        try:
            ...
            logger.debug("transfer_cert_postal process_loop_ret result has problem:%s, error:%s" % (result, traceback.format_exc()))
...
def get_command(tasks):
    ...
    for t in tasks:
        co = {'o_path':t.get('o_path',''),'d_path': t.get('d_path',''), 'save_name': t.get('save_name'),'task_id': t.get('_id','')}
        commands.append(co)
