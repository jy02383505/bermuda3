...
def doloop_url(devs, tasks, Interface, connect_timeout=5, response_timeout=8):

    """
    调用asyncore，创建信道，与设备通过socket连接,端口51108
    :param devs:
    :param command:
    :return:
    """
    ...
    for dev in devs:
        clients.append(AioClient(dev['host'], port, Interface, command, connect_timeout,response_timeout))
    ...
    for r in results:
        ...
        if response_body:
            try:
                ...
                logger.debug ("pre_ip_test:%s, response_code_test:%s, response_body:%s" % (r.get('host'), response_code, response_body))
            except Exception:
...
def process_loop_ret(http_results, dev_map):
    ...
    for result in http_results:
        try:
            ...
            logger.debug("cert_query_postal process_loop_ret result has problem:%s, error:%s" % (result, traceback.format_exc()))
        ...
        try:
            ...
            if _code == 200:
                ...
                error_result[host] = getPostStatus(dev, _code, total_cost, connect_cost, response_cost, int(a_code))
...
def retry(devs, tasks, results_faild_dic, Interface):
    """
    失败后重新下发命令
    :param devs:
    :param command:
    ...
    for retry_count in range(RETRY_COUNT):
        ...
        ret, ret_faild = doloop_url(devs,tasks,Interface,connect_timeout, response_timeout)
        for result in ret:
            try:
                host, _body, a_code, total_cost, connect_cost, response_cost = result.split('\r\n')
            except Exception:
            ...
            try:
                ...
                logger.debug("retry host_test1:%s, code_test1:%s, r_code_test1:%s" % (host, _body, a_code))
            except Exception:
...
def get_command(tasks):
    ...
    for t in tasks:
        co = {'query_cert_type':t.get('query_cert_type',''),'query_path': t.get('query_path',''), 'query_config_path': t.get('query_config_path', ''), 'query_cert_name': t.get('query_cert_name'),'task_id': t.get('_id',''),  'query_type': t.get('query_type', '')}
        commands.append(co)
