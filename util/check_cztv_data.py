...
def preload_callback(channel, callback_body, info={}):
    ...
    try:
        ...
        if channel.get('callback', {}).get('url'):
            ...
            # curls = ["http://192.168.7.24:8080/examples/servlets/mh"]
            curls = ["http://127.0.0.1:3000/report"]
            # if ca_url:
            #     curls.append(ca_url)
...
def report_task(channel_code, region_channel_map, url):
    logger.debug("send region report, devs: {0}".format(url.get('_id')))
...
#check callback run status
def getCallbackData(url):
    print()
    call_obj=db.callback.find_one({"task_id": url["task_id"],"reponse":{"$ne":"ok"},"create_time":{"$gte":url["created_time"]}})
    #.count()
    # count = db.preload_url.find({"task_id": url["task_id"]}).count()
    try:
        count = len(call_obj)
        #重新发送的任务，提交任务的时间与发送时间可能不同
        logger.debug("%s:%s,%s--%s"%(url["task_id"],count,url["created_time"],call_obj["create_time"]))
        return False
    except:
        logger.debug("%s:%s"%(url["task_id"],url["created_time"]))
        return True


def check_callbck():
    pre_time = datetime.datetime.strptime("2015-11-02 00:00:00", "%Y-%m-%d %H:%M:%S")
    last_time = datetime.datetime.strptime("2015-11-02 13:00:00", "%Y-%m-%d %H:%M:%S")
    ...
    for url in channel_urls:
        if getCallbackData(url):
            num +=1
            print(url["task_id"])
    print("url count:%s"%need_count)
    print("pro count %s"%num)
...
def run():
    ...
    pre_time = datetime.datetime.strptime("2015-09-18 14:00:00", "%Y-%m-%d %H:%M:%S")
    last_time = datetime.datetime.strptime("2015-09-21 08:00:00", "%Y-%m-%d %H:%M:%S")
    ...
    channel_urls = db.preload_url.find(ind_url_dic)
    logger.debug("url count:%d" % channel_urls.count())
    num = 0
    tt_list =[]
    for url in channel_urls:
        # try:
        #     # count = db.callback.find({"task_id": url["task_id"],"create_time": {"$gte": pre_time, "$lt": last_time}}).count()
        #     # if count > 0:
        #     #     pass
        #     # else:
        #     #     logger.debug(url)
        #     # print url["task_id"]
        #     # if url["task_id"] not in tt_list:
        #     #     tt_list.append(url["task_id"])
        #     # else:
        #     #     logger.debug(url)
        #     # if getCallbackData(url):
    ...
    logger.debug("process url num:%d" % num)
